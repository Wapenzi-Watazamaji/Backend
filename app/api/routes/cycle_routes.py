...
from app.schemas.cycle import (
    FormTemplateRead, CycleEntryCreate, CycleEntryUpdate, CycleEntryRead,
...
async def get_cycle_entry_form_template(
    ...
    template = await cycle_repository.get_active_form_template(db, FormContext.CYCLE_ENTRY)
    if not template:
...
async def get_symptom_form_template(
    ...
    template = await cycle_repository.get_active_form_template(db, FormContext.CYCLE_SYMPTOM)
    if not template:

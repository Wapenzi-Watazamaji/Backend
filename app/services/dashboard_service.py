...
async def get_dashboard_summary(
    ...
    total_alerts = labour_alerts_count + postpartum_alerts_count
    
    # ancVisitsToday
    stmt = select(func.count(ScheduledVisit.id)).where(
...
async def get_unified_alerts(db: AsyncSession, facility_id: uuid.UUID) -> list[DashboardAlert]:
    alerts = []
...
async def get_patient_directory(db: AsyncSession, facility_id: uuid.UUID, search_term: Optional[str] = None) -> list[PatientDirectoryItem]:
    ...
    for user, profile in results.all():
        ...
        directory.append(PatientDirectoryItem(
            ...
            assignedClinicianName=None
        ))
        
    return directory


async def get_global_timeline(db: AsyncSession, facility_id: uuid.UUID) -> list[TimelineItem]:
    events = []
